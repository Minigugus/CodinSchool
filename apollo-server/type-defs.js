...
const importerSchema = (fichier, dossier = __dirname) => {
  ...
  const importer = (fichier, dossier) => {
    ...
    if (!charge.has(chemin))
    {
      ...
      charge.set(chemin, contenu.replace(/# ?import (\*|(.*)) from ('|")([^"']+)('|");?/g, '').trim())
      // On charge les importations
      const regex = /# ?import (\*|(.*)) from ('|")([^"']+)('|");?/g;
      for (let i; (i = regex.exec(contenu)) != null;)
        importer(i[4], dossier);
    }
  };
