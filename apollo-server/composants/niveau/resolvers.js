  ...
  Mutation: {
    creerNiveau(_, { niveau }) {
      return bdd.transaction(async transaction => {
        ...
        niveau.position = dernierNiveau + 1
    ...
    async reorganiserNiveaux(_, { niveaux }) {
      await bdd.transaction(async transaction => {
        ...
        else if (niveaux.length > nombreNiveaux)
          throw new Error('Au moins un Niveau n\'existe pas.');
        return Promise.all(niveaux.map((id, position) => Niveau.update({ position }, { where: { id }, transaction })));
      });
