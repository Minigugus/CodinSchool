...
let utilisateurs_id = 0, roles_id = 0;
...
module.exports = {
	...
	creer_utilisateur(donnees_creation) {
		...
		bdd.get('utilisateurs').push(utilisateur).write();
		return bdd
			.get('utilisateurs')
	...
	authentifier(email, mot_de_passe) {
		const utilisateur = bdd.get('utilisateurs').find({ email_primaire: email }).value();
	...
	ajouter_role(id_utilisateur, id_role) {
		if (!bdd.get('roles').find({ id: id_role }).value())
			throw new Error(`Rôle ${id_role} introuvable.`);
		if (!bdd.get('utilisateurs').find({ id: id_utilisateur }).value())
